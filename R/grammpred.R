...
grammpred=function(xtrain,xtest,kernel,sigma,degree,scale) {
  {if (kernel=="linear") {
    K=kernelMatrix(vanilladot(),as.matrix(xtest),as.matrix(xtrain))
  } else if (kernel=="polynomial") {
    K=kernelMatrix(polydot(degree = degree,scale=1, offset=0),as.matrix(xtest),as.matrix(xtrain))
  } else if (kernel=="radial") {
    K=kernelMatrix(rbfdot(sigma = sigma),as.matrix(xtest),as.matrix(xtrain))
  } else if (kernel=="sigmoid") {
    K=kernelMatrix(tanhdot(scale=scale,offset=1),as.matrix(xtest),as.matrix(xtrain))
  }
  else if (kernel=='clinical_cat'){
    int=lapply(1:dim(xtrain)[2], function(a) outer(xtest[,a],xtrain[,a],'=='))
    clinical_mat=Reduce('+',int)/length(int)
    K=as.kernelMatrix(clinical_mat)
  }
