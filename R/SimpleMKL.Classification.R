...
SimpleMKL.classification=function(k,outcome,penalty,tol=10^(-4),max.iters=1000){
  inner=function(Ddag,gammadag,k,J,penalty){
    ...
    while(Jdag<J&sum(Ddag<0)>0){
      ...
      model=ipop(rep(-1,length(outcome)),h,t(outcome),0,rep(0,length(outcome)),rep(penalty,length(outcome)),0)
      Jdag=-1/2*sum(h*primal(model)%*%t(primal(model)))+sum(primal(model))
    }
  ...
  while(epsilon>tol&iters<max.iters&sum(gamma==1)==0){
   # tic()
    ...
    h=kk*(outcome%*%t(outcome))
    model=ipop(rep(-1,length(outcome)),h,t(outcome),0,rep(0,length(outcome)),rep(penalty,length(outcome)),0)
    J=-1/2*sum(h*primal(model)%*%t(primal(model)))+sum(primal(model))
    dJ=sapply(1:length(k), function(a) -1/2*sum(k[[a]]*outcome%*%t(outcome)*primal(model)%*%t(primal(model))))
    ...
    step_opt=optim(innersol$stepsize/2,obj,outcome=outcome,penalty=penalty,direction=innersol$direction,gamma=innersol$gamma,lower=0,upper=innersol$stepsize,method='L-BFGS-B')
