...
kernels.gen=function(data,train.samples,kernels,degree,scale,sigma){
  ...
  if(length(kernels)==1){
  K.train= gramm(x=train.data,degree=degree,scale=scale,sigma=sigma, kernel=kernels)
  trace.train=trace.kernel(K.train)
  K.train=unit.trace(K.train)
  K.test=grammpred(xtrain=train.data,xtest=test.data,scale=scale,degree=degree,sigma=sigma,kernel=kernels)
  K.test=K.test/trace.train
 }else{
  K.train=lapply(1:length(kernels),function(i){
                   gramm(x=train.data,degree=degree[i],scale=scale[i],sigma=sigma[i], kernel=kernels[i])})
  trace.train=unlist(lapply(K.train, trace.kernel))
  K.train=lapply(K.train,unit.trace)
  K.test=lapply(1:length(kernels),function(i){
                  grammpred(xtrain=train.data,xtest=test.data,scale=scale[i],degree=degree[i],
