...
gramm=function(x,kernel,sigma,degree,scale){
  {if (kernel=="linear") {
    K=kernelMatrix(vanilladot(),as.matrix(x))
  } else if (kernel=="polynomial") {
    K=kernelMatrix(polydot(degree = degree, offset=0, scale = 1),as.matrix(x))
  } else if (kernel=="radial") {
    K=kernelMatrix(rbfdot(sigma = sigma),as.matrix(x))
  } else if (kernel=="sigmoid") {
    K=kernelMatrix(tanhdot(scale=scale,offset=1),as.matrix(x))
  }
